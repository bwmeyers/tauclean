...
def run_clean(taus, data, clean_kwargs):
    """
    Utility function that just runs the clean algorithm using the
    multiprocessing module, as done in the `tauclean` script
    """
    result_list = []
...
def test_dm_delay_100():
    dm = 100
    flo = 1.4
    fhi = 7 * np.sqrt(518601.0 / 273601.0) / 5  # solved so that dt = 100 ms
    dt = dm_delay(dm, flo, fhi)
